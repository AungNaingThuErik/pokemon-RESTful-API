...
const cache = require("./cache"); //call cache function
...
exports.fetchData = async (key, endpoint) => {
  ...
  try {
    ...
    //make a request for a pokimon with a given name or id from pokiapi
    ...
    if (response.statusCode !== undefined && response.statusCode !== 200) {
      ...
      const {
        data: { id, name, types, location_area_encounters, stats },
      } = response;
      ...
      if (encounters_res) {
        encounter_loc = encounters_res.data
          .map((enctr) => ({
            ...
            methods: enctr.version_details.map((d) =>
              d.encounter_details.map((ed) => ed.method.name)
            ),
          }))
